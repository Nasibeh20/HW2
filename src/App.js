...
function App() {
  ...
  const [userConcentrationThreshold, setUserConcentrationThreshold] = useState(0.7); // Set an initial threshold
    ...
    >
      ...
      <div style={{ 'maxHeight': '7vh' }}>
        ...
        <input
          ...
          max={0.99}
          step={0.01}
          value={userConcentrationThreshold}
          onChange={(e) => setUserConcentrationThreshold(parseFloat(e.target.value))}
        />
        <label for="concentration">{"%" + (userConcentrationThreshold * 100 )}</label>
      </div>
      <div
        className={'shadow'}
        style={{ 'height': '50vw', 'width': 'calc(49vw - 10em)', 'maxHeight': '80vh', 'display': 'inline-block', 'margin': '3px' }}
      >
        <Particle3D
          ...
          userConcentrationThreshold={userConcentrationThreshold}
        />
      </div>
      <div
        className={'shadow'}
        style={{ 'height': '50vw', 'width': 'calc(48vw - 10em)', 'maxHeight': '80vh', 'display': 'inline-block', 'margin': '3px' }}
      >
        <LinkedViewD3
          colorRange={colorRange}
          data={particleData}
      ...
      >
        <div style={{ 'width': '100%', 'maxHeight': '5em', 'margin': '0px' }}>
          ...
          <p style={{ 'margin': '0px', 'padding': '0px' }}>{"Use buttons to change orientation of plane"}</p>
        </div>
