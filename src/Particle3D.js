...
export default function Particle3D(props) {
    ...
    useEffect(() => {
        if (props.bounds !== undefined & camera !== undefined) {
            ...
            const concentrationThreshold = props.userConcentrationThreshold * d3.max(props.data, d => d.concentration);
            const filteredData = props.data.filter(d => d.concentration > concentrationThreshold);
    ...
    }, [camera, props.data, props.userConcentrationThreshold]);
    ...
    useEffect(() => {
        ...
        // Color scale for brushed points
        const saturatedColorScale = d3.scaleSymlog()
            .domain([0, concentrationThreshold])
            .range(['purple', 'blue']);

        const desaturatedColorScale = d3.scaleSymlog()
            .domain([concentrationThreshold, bounds.maxC])
            .range(['blue', 'lightblue']);

       

        // function isBrushed(d) {
        //     return d.concentration > concentrationThreshold;
        // }

        function getColor(d) {
            let alpha = 1 - d.concentration / props.bounds.maxC; // Higher concentration, less transparent
            var c = d.concentration > props.userConcentrationThreshold ? saturatedColorScale(d.concentration) : desaturatedColorScale(d.concentration);
            c = d3.color(c);
    ...
    }, [scene, props.brushedCoord, props.brushedAxis, props.userConcentrationThreshold])
